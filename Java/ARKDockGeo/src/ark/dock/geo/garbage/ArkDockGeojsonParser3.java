...
public class ArkDockGeojsonParser3 implements ArkDockJsonUtils, ArkDockGeojsonConsts {
    ...
    public static void parse(Reader r, GeojsonBuilder builder) throws IOException, ParseException {
        ...
        DustGenAgent agent = new DustGenAgent() {
            int featureCount = 0;

            @Override
            public DustResultType agentAction(DustAgentAction action) throws Exception {
                if ((action == DustAgentAction.PROCESS) && "Feature".equals(ctx.getParam())) {
                    ++featureCount;
                }
            ...
            public String toString() {
                return "Feature count = " + featureCount;
            }
        };
        JsonContentHandlerAgent h = new JsonContentHandlerAgent(agent, ctx);
        p.parse(r, h);
        DustGenLog.log("Agent result", agent);
